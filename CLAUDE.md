...
- **`serialx` is pinned by HA core** (`package_constraints.txt`): `==1.7.0` (HA 2026.5.0), `==1.7.3` (HA 2026.5.4), `==1.8.0` (HA 2026.6.0). HA installs manifest requirements with `-c package_constraints.txt`, so an exact pin in `manifest.json` is unsatisfiable on any HA version that carries a different pin. `manifest.json` carries `>=1.7.0,<2`; this is safe because the 1.7→1.8 change only touched the async write API and this integration uses the sync API throughout. `pyproject.toml` pins `==1.8.0` for dev/CI reproducibility. Dependabot is configured to ignore standalone `serialx` bumps (see `.github/dependabot.yml`). When upgrading HA, check `package_constraints.txt` and bump the dev pin in `pyproject.toml` to match the new HA pin.
