...
async def test_state_triggered_print_automation(printer_with_ha, automation_config) -> None:  # type: ignore[no-untyped-def]
    ...
    await ha_env.state_simulator.trigger_state_change("sensor.test_sensor", "off", "on")
...
async def test_multiple_state_triggers(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    multi_trigger_config = {
        ...
        "trigger": [
            {"platform": "state", "entity_id": "sensor.trigger1", "to": "active"},
            {"platform": "state", "entity_id": "sensor.trigger2", "to": "active"},
        ],
        "action": {
            "service": "escpos_printer.print_text",
            "data": {"text": "Multi-trigger activation!", "align": "center"},
        },
    }
    ...
    await ha_env.state_simulator.trigger_state_change("sensor.trigger1", "inactive", "active")
    ...
    await ha_env.state_simulator.trigger_state_change("sensor.trigger2", "inactive", "active")
...
async def test_conditional_print_automation(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    conditional_config = {
        ...
        "trigger": {"platform": "state", "entity_id": "sensor.main_trigger", "to": "activate"},
        "condition": {
            "condition": "and",
            "conditions": [
                {"condition": "state", "entity_id": "sensor.condition1", "state": "enabled"},
                {"condition": "state", "entity_id": "sensor.condition2", "state": "ready"},
            ],
        },
        "action": {
            "service": "escpos_printer.print_text",
            "data": {"text": "Conditions met - printing!", "bold": True},
        },
    }
    ...
    await ha_env.state_simulator.trigger_state_change("sensor.main_trigger", "idle", "activate")
...
async def test_notification_triggered_print(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    await ha_env.notification_tester.send_notification(
        message=test_message, title="Test Notification"
    )
...
async def test_automation_sequence_with_multiple_services(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    sequence_config = {
        ...
        "trigger": {"platform": "state", "entity_id": "sensor.sequence_trigger", "to": "start"},
        "action": [
            {
                "service": "escpos_printer.print_text",
                "data": {"text": "Header", "align": "center", "bold": True},
            },
            {"service": "escpos_printer.feed", "data": {"lines": 1}},
            {
                "service": "escpos_printer.print_qr",
                "data": {"data": MockDataGenerator.generate_qr_data(), "size": 4},
            },
            {"service": "escpos_printer.feed", "data": {"lines": 1}},
            {"service": "escpos_printer.print_text", "data": {"text": "Footer", "align": "center"}},
            {"service": "escpos_printer.cut", "data": {"mode": "full"}},
        ],
    }
    ...
    await ha_env.state_simulator.trigger_state_change("sensor.sequence_trigger", "idle", "start")
...
async def test_automation_with_template_data(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    template_config = {
        "id": "template_test",
        "alias": "Template Test",
        "trigger": {"platform": "state", "entity_id": "sensor.temperature", "to": "changed"},
        "action": {
            "service": "escpos_printer.print_text",
            "data": {
    ...
    # Trigger automation
    await ha_env.state_simulator.trigger_state_change("sensor.temperature", "20", "25")
...
async def test_automation_error_handling(printer_with_ha) -> None:  # type: ignore[no-untyped-def]
    ...
    error_test_config = {
        ...
        "trigger": {"platform": "state", "entity_id": "sensor.error_trigger", "to": "trigger"},
        "action": {
            "service": "escpos_printer.print_text",
            "data": {"text": "This should handle errors gracefully", "align": "center"},
        },
    }
    ...
    # Trigger automation
    await ha_env.state_simulator.trigger_state_change("sensor.error_trigger", "idle", "trigger")
