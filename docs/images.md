...
| Image prints at about two-thirds of the expected width | No profile/width selected, so images fall back to 384px on an 80mm head. Set "Paper width in pixels" in the options, or run the [calibration wizard](calibration.md). See [How the target width is chosen](#how-the-target-width-is-chosen). |
