...
- **Status polling competes with prints.** Bluetooth Classic accepts one client at a time, so each status check is a real RFCOMM open and many cheap printers audibly beep on every connect. Set **Status check interval** to **60 seconds or longer**, or leave at `0` to disable polling and rely on print success/failure for liveness. Aggressive polling beeps the printer, drains battery on portable models, and contends with in-flight print jobs. This is enforced: for Bluetooth entries, `0` disables polling, `1`–`59` is rejected with a form error, and `60` or higher is accepted.
